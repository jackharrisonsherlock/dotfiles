#  Prompt
PROMPT='$(_user_host)%{$fg[blue]%}%c $(git_prompt_info)%{$reset_color%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}%(?.%F{magenta}.%F{red}❯%F{magenta})❯%f '

local _return_status="%{$fg[magenta]%}%(?..⍉ )%{$reset_color%}"
